	...
	if($_GET['q']=='new'){
		...
		if($_GET['pla']==''){
			$sql="SELECT DISTINCT x.*,
					...
					END as hn
					FROM
					(SELECT appoint.id as appoint_id,patients.id as id,appoint.cid,appoint.hn as hn_app,appoint.ptname,appoint.placecode,appoint.placename,date_format(appoint.date_app,'%d-%m-%Y') as dateapp,appoint.time_app,hospital.HOSNAME as hosname,date_format(appoint.date_created,'%d-%m-%Y %H:%i:%s') as datecreate,appoint.referout_no,appoint.date_app ,appoint.visit_opd,appoint.user_app,status_app,total_app.placecode_main,appoint.visit_appoint,refer_appoint,appoint.date_created
					FROM appoint 
					LEFT JOIN total_app ON appoint.placecode=total_app.placecode
					...
					where ";
					if($app_status=="1"){
						$sql.="status_app ='Y' and ";
					}elseif($app_status=="0"){
						$sql.="";
					}
					$sql.="appoint.del_flag <> 'Y') x					
					WHERE x.date_app='".$date_app."'	ORDER BY x.date_created DESC";
		}else{
			$sql="SELECT DISTINCT x.*,
					CASE
						WHEN x.hn_app='' OR x.hn_app is NULL THEN (select hn from patients WHERE patients.cid=x.cid AND patients.hospcode=x.user_app)
