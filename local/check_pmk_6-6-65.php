<?php
	include("../config/conn_refer.php");
		...
		/*
$sql="SELECT id,hn,placecode,date_app,referout_no,visit_appoint,refer_appoint from appoint WHERE (hn is not null or hn<>'') and DATE_FORMAT(date_app,'%Y-%m-%d')>=date_format(now(),'%Y-%m-%d') AND ((refer_appoint ='' or refer_appoint is NULL) or (visit_appoint='' or visit_appoint is null) or (visit_opd='' or visit_opd is null))";
