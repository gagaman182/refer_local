	...
	if($_GET['q']=='new'){
		...
		if($_GET['pla']==''){
			$sql="SELECT DISTINCT x.*,
					...
					WHERE date_format(x.date_app,'%Y-%m-%d')>=date_format(now(),'%Y-%m-%d')
			...
			$sql="SELECT DISTINCT
					...
					LEFT JOIN patients ON appoint.cid=patients.cid AND appoint.user_app=patients.hospcode 
					left JOIN hospital ON appoint.user_app=hospital.HOSCODE5 
					where total_app.placecode_main ='".$_GET['pla']."' and appoint.del_flag <>'Y') x 
					WHERE date_format(x.date_app,'%Y-%m-%d')>=date_format(now(),'%Y-%m-%d')
					ORDER BY x.datecreate DESC";
		}
		//echo $sql;
		...
		while($rs=mysql_fetch_array($query)){
			...
			$visit_status=chk_appoint_status($hn,$rs['date_app'],$rs['placecode']);
			if($hn==null){
				$app_status="0";
				$refer_status="0";
				$visit_status="0";
				...
				$app_status=chk_app_status($hn,$rs['placecode_main'],$rs['date_app']);
			}
			...
			if($_GET['app_status']=="0"){
				...
				if($refer_status=='1'){
					$a['refer_status']='<i class="far fa-check-circle fa-2x text-success"></i>';			
				}else{
					$a['refer_status']='<i class="far fa-times-circle fa-2x text-warning"></i>';
				}

				if($visit_status=='1'){
					$a['visit_status']='<i class="far fa-check-circle fa-2x text-success"></i>';			
				}else{
					$a['visit_status']='<i class="far fa-times-circle fa-2x text-warning"></i>';
				}
				array_push($data,$a);
		...
		$sql="SELECT appoint.placename,appoint.time_app,referout_no,date_format(appoint.date_app,'%d-%m-%Y'),user_app, ptname,cid,total_app.placecode_main FROM appoint LEFT JOIN total_app ON appoint.placecode=total_app.placecode WHERE appoint.del_flag <>'Y' and date_format(appoint.date_app,'%Y-%m-%d')='".$_GET['date_app']."' and total_app.placecode_main ='".$_GET['pla']."'";
