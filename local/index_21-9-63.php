										...
										<table id="tbl_ptapp" class="table table-sm" data-toolbar="#toolbar"  data-toggle="table" data-cache="false" data-search="true" data-show-print="false" data-url="get_appoint.php?q=app&pla=&app_status=0" data-show-export="true" data-pagination="true" data-row-style="rowStyle" data-export-types="['excel']" data-locale="th-th" data-auto-refresh="true">
											<thead class="thead-light">	
												<tr>
													...
													<th data-field="refer_status">ลงรับ Refer</th>
													<th data-field="visit_status">Visit บัตรนัด</th>
					...
					<div class="tab-pane fade p-1" id="m-report" role="tabpanel" aria-labelledby="m-report-tab">
						<div class="row">
							<div class="col-md-6">
								<div class="panel  panel-primary">
									<div class="panel-heading">
										<h3 class="panel-title">จำนวนผู้ป่วย Refer นัด แยกรายโรงพยาบาล</h3>											
									</div>
									<div class="panel-body p-1">
...
<script type="text/javascript">
	...
	$(document).ready(function() {		
		setInterval(update_appoint, 300000);
		...
		if(id!=null){ // ดึงข้อมูลทั่วไปของคนไข้ กรณีทำบัตรใหม่ ดึงจากการบันทึกเพิ่มเติม
			...
			success: function(data) {
				...
				if(data[0]!=null){
					...
					$("#father_lname").val(data[0].father_lname);
					$("#mother").val(data[0].mother);
					$("#master_lname").val(data[0].mother_lname);
					$("#wifename").val(data[0].wifename);
					$("#wife_lname").val(data[0].wife_lname);
					$('#tel').val(data[0].tel);	
					$('#tel_connect').val(data[0].tel_connect);	
	...
	function total(){
		var options = {
			...
			title: {
			  text: 'จำนวน นัด ผู้ป่วย Referแยกรายรพ. ',
			},
			subtitle: {
	...
	function total_place(){
		var options = {
			...
			title: {
			  text: 'จำนวน นัด ผู้ป่วย Referแยกรายแผนก ',
			},
			subtitle: {
				//text: 'ข้อมูลตั้งแต่วันที่ 01-10-2016'
			},
