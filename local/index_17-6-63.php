							...
							<div class="col-md-12">
								<div class="panel  panel-primary">
									...
									<div class="panel-body">	
										<table id="tbl_ptlist" class="table" data-toolbar="#toolbar_regis" data-toggle="table" data-cache="false" data-search="true" data-show-print="false" data-url="get_appoint.php?q=new&status=0" data-show-export="true" data-pagination="true" data-locale="th-th" data-export-types="['excel']" data-page-size="5" data-auto-refresh="true">
...
*/
	...
	$("#btn_slot").on('click',function(){
		...
		$.ajax({
			url: "edit_slot.php",
			type: "POST",
			data: {placecode:pla,total_app:total_app,time_app:time_app},
			error: function (request, status, error) {
				console.log(request.responseText);
			},
			success:function(data){
			...
			success: function(data) {
				...
				for (i = 0; i < datal; i++) {
					if(data[i].userid=='' || data[i].userid==null){ // ส่ง sms กรณีทำ refer คนไข้ใหม่
						...
						return false;
					}else{ // ส่ง line กรณีทำบัตร online
				  //console.log(data[i].ptname);
					  message='ชื่อ สกุล : '+data[i].prename+data[i].ptname+' '+data[i].lname+'\nHN : '+data[i].hn+'\n';
					  //console.log(message);
					  var result=pushtext(data[i].userid,message,'template','https://liff.line.me/1654185694-BrZbrl1G','Register Online');		// function ส่ง line			
						console.log('text:'+result);
						if(result=="1"){
							//console.log('aaa');
							$.ajax({
