	...
	if($_GET['q']=='new'){
		...
		if($_GET['pla']==''){
			$sql="SELECT DISTINCT x.*,
					...
					where ";
					if($app_status=="1"){
						$sql.="status_app is null and ";
					}elseif($app_status=="0"){
						$sql.="";
					}
					$sql.="appoint.del_flag <> 'Y') x					
			...
			$sql="SELECT DISTINCT x.*,
					...
					$sql.="appoint.del_flag <> 'Y' and total_app.placecode_main='".$_GET['pla']."') x
		...
		while($rs=mysql_fetch_array($query)){
			...
			*/
			...
			if($rs['refer_appoint']=='1'){
				$a['refer_status']='<i class="far fa-check-circle fa-2x text-success"></i>';			
			}else{
				$a['refer_status']='<i class="far fa-times-circle fa-2x text-warning"></i>';
			}
			if($rs['visit_appoint']=='1'){
				$a['visit_status']='<i class="far fa-check-circle fa-2x text-success"></i>';			
			}else{
				$a['visit_status']='<i class="far fa-times-circle fa-2x text-warning"></i>';
			}
